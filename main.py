# Arcade Style Duck Shooter Game
# Kris Meals - Started 11/23/2022
# Finished 12/01/2022
# Version 1.0
import pygame
import math
...
def draw_score():
    ...
    time_text = font.render(f'Time Elapsed: {time_past}', True, 'black')
...
def draw_level(coords):
    ...
    for i in range(len(coords)):
        for j in range(len(coords[i])):
            target_rect[i].append(pygame.rect.Rect((coords[i][j][0] + 20, coords[i][j][1]),
                                                   (60 - i * 12, 60 - i * 12)))
...
def draw_menu():
    global game_over, pause, mode, level, menu, time_past, total_shots, points, ammo
...
def draw_game_over():
    global clicked, level, pause, game_over, menu, total_shots, time_past, time_remaining
    if mode == 0:
        display_score = time_past
    else:
...
def draw_pause():
    global level, pause, menu, points, time_remaining, time_past, total_shots, clicked, new_coords
